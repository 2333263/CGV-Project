...
class PointerLockControls extends EventDispatcher {
	...
	constructor( camera, domElement ) {
		...
		function onMouseMove( event ) {
			...
			if(Math.abs(Math.abs(movementX)-Math.abs(prevMoveX))>150 && prevMoveX!=-999 &&movementX!=0){
				movementX=(window.innerWidth/movementX)-2.3

		}
			// this if statement is a work around to a bug in chrome where it wouldnt read the mouse position properly
			if( Math.abs(Math.abs(movementY)-Math.abs(prevMoveY))>150 && prevMoveY!=-999 &&movementY!=0){
